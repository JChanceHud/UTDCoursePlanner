...
class course {
	...
	function getTimeslots($daystring, $timestring, $roomstring){
		if(strlen($daystring) < 1 || strlen($timestring) < 1)
			return array();
		$timeslots = array();
		for ($x = 0; $x < count(explode($daystring, "|")); $x++) {
			$currentTimestring = explode($timestring, "|")[$x];
			$currentDaystring = explode($daystring, "|")[$x];
			$currentRoomstring = explode($roomstring, "|")[$x];

			$currentTimestring = str_replace(" ", "", $currentTimestring);
			$base = strlen($currentTimestring)-1;
			while(substr($currentTimestring, $base, 1) !== "-"){
				$base--;
				if($base < 0){
					return array();
				}
			...
			$endTime = $this->parseTime(substr($currentTimestring, $base, strlen($currentTimestring)-$base)); //get the ending time

			$oldBase = $base-1;
			$startTime = $this->parseTime(substr($currentTimestring, 0, $oldBase)); //get the ending time
